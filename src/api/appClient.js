...
function readLocalStore() {
  ...
  if (existing) {
    ...
    const draftsById = new Map((store.Draft || []).map((draft) => [draft.id, draft]));
    store.DraftBoardItem = (store.DraftBoardItem || []).map((item) => ({
      ...item,
      league_id: item.league_id || draftsById.get(item.draft_id)?.league_id || null,
    }));
    return store;
  }
...
async function localSubmitDraftPick({ draft_id, player_id, auto_pick = false } = {}) {
  ...
  const boardItems = (await entities.DraftBoardItem.list()).filter((item) =>
    item.player_id === player_id &&
    (item.league_id === draft.league_id || item.draft_id === draft_id)
  );
...
const localFunctions = {
  ...
  async submit_pick(payload = {}) {
    ...
    if (payload.league_id && payload.player_id) {
      const boardItems = (await entities.DraftBoardItem.list()).filter((item) =>
        item.league_id === payload.league_id && item.player_id === payload.player_id
      );
      await Promise.all(boardItems.map((item) => entities.DraftBoardItem.delete(item.id)));
    }
...
const playerStats = isSupabaseConfigured
  ? {
      ...
      async listWeeklySummaries({ playerId } = {}) {
        ...
        const { data, error } = await supabase
          .from("player_week_stats")
          .select("id,player_id,season_year,week,team,opponent_team,fantasy_points")
          .eq("player_id", playerId)
  ...
  : {
      async getAggregate({ playerId, seasonYear = null } = {}) {
        return localPlayerSeasonAggregate(readLocalStore(), playerId, seasonYear);
      },
      async listWeeklySummaries({ playerId } = {}) {
        const store = readLocalStore();
        return (store.PlayerWeek || [])
          .filter((week) => week.player_id === playerId)
          .map(normalizePlayerWeekSummary)
          .sort((a, b) => {
      ...
      async getWeekDetail({ playerWeekId, playerId, seasonYear, week } = {}) {
        const store = readLocalStore();
        const detail = (store.PlayerWeek || []).find((row) =>
          playerWeekId
            ? row.id === playerWeekId
            : row.player_id === playerId && Number(row.season_year) === Number(seasonYear) && Number(row.week) === Number(week)
        );
        return detail ? { ...(detail.raw_stats || {}), ...detail, season: detail.season_year } : null;
      },
    };
